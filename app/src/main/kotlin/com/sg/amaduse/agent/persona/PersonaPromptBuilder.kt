...
internal object PersonaPromptBuilder {
    ...
    private fun buildGenericPrompt(
        ...
        append("可用工具包括：test_voice、test_emotion、set_alarm、add_memo、web_search、create_local_file。add_memo 会在 Android 官方 Calendar 中创建日历任务/事项，创建前必须具备标题、开始时间、结束时间；缺少信息时先追问，不要调用工具。用户要求搜索、联网查询、最新信息、新闻或事实核实时，调用 web_search 后基于搜索结果回答，并尽量给出来源。用户要求创建或保存 txt/md/csv/docx/xlsx 文件时，必须先具备文件名、格式和内容，缺少任何一项先追问，然后调用 create_local_file。")
    ...
    private fun StringBuilder.appendContext(context: PromptRuntimeContext) {
        ...
        appendLine("<AgentCapabilities>你可以调用以下工具来执行操作：test_voice（测试语音合成）、test_emotion（测试 Live2D 表情）、set_alarm（按北京时间直接创建系统闹钟；相对时间优先传 relative_minutes/relative_hours，绝对时间传北京时间 hour/minute）、add_memo（在 Android 官方 Calendar 中创建日历任务/事项）、web_search（联网搜索公开网页并返回结果）、create_local_file（创建并保存 txt/md/csv/docx/xlsx 到本地 Documents/Amaduse）。创建日历任务前必须具备 title、start_at、end_at 三项完整信息，时间格式为北京时间 yyyy-MM-dd HH:mm；缺少任何一项时先向用户追问，不要调用工具。创建本地文件前必须具备 file_name、format、content；缺少任何一项时先向用户追问，不要调用工具。用户要求搜索、联网查询、最新信息、新闻或事实核实时，调用 web_search，回答时基于搜索结果并尽量列出来源链接。满足全部信息后再调用工具；闹钟、日历任务和本地文件创建不需要用户再手动确认。</AgentCapabilities>")
        appendLine("</RuntimeContext>")
    }
